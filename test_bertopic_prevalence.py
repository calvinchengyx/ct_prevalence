### INPUT: parquet file with ct classification results
    # input file: "calvin_posts.parquet"
# key vars: "embed_id", "claim_text" (i used embed_id to link with claim_id, claim_embedding, and claim text)

#### processing steps:
# 1. embed claim_text using sentence-transformers embedding model - i used Qwen/Qwen3-Embedding-0.6B
# 2. use bertopic to cluster claims
# 3. create a new df, with `embed_id`, `claim_txt`, and `topic_id` columns
...
# 2. "5_claim2topic_doctopic_{date}_v{number}.json" with embed_id, claim_text, topic_id columns, so each embed_id is associated with each topic_id
...
# Try to import RAPIDS cuML UMAP and HDBSCAN, fall back to regular versions if not available
try:
    from cuml import UMAP
    from cuml.cluster import HDBSCAN
    print("Using RAPIDS cuML UMAP and HDBSCAN for GPU acceleration")
    RAPIDS_AVAILABLE = True
except ImportError:
    from umap import UMAP
    from hdbscan import HDBSCAN
    print("RAPIDS cuML not available, using standard UMAP and HDBSCAN")
    RAPIDS_AVAILABLE = False

# you have to type the dataset name as an argument when running the script
DATASET = sys.argv[1]
# arguments are: 'moonlanding' '911' 'covid' 'alien' 'nwo'

### Configuration
SERVER = "/VData/scro4316/ct_prevalence/"
INPUT = f"{SERVER}/calvin_posts.parquet"
OUTPUT_PATH = f"{SERVER}/results"
EMBEDDINGS_OUTPUT_PATH = f"{SERVER}/embeddings"
CHUNK_SIZE = 10000
VERSION = 1
DATE = time.strftime("%Y%m%d")
# Use all available GPUs starting from GPU 0
AVAILABLE_GPUS = "0,1,2,3"  # Use first 4 GPUs, adjust based on your needs

### 1. Load the INPUT data
print("Loading input data...")

df = pd.read_parquet(INPUT)
df = df[df['topic']==DATASET]
# convert IDs to strings for better matching later
# df['content_id'] = df['content_id'].astype(str)
# df['embed_id'] = df['embed_id'].astype(str)
...
docs = df['claim_txt'].tolist()  # Unique texts only
...
def get_hdbscan(min_cluster_size, min_samples):
    if RAPIDS_AVAILABLE:
        # Use RAPIDS cuML HDBSCAN for GPU acceleration
...
def get_topic_model(min_cluster, umap_model, hdbscan_model):
...
def get_low_quality_topics(evaluation_results, coherence_threshold=0.5, silhouette_threshold=0.0):
    """
...
def main():
    ...
    # Set GPU devices for RAPIDS cuML if available
    if RAPIDS_AVAILABLE:
        os.environ['CUDA_VISIBLE_DEVICES'] = AVAILABLE_GPUS
        print(f"Using GPU devices: {AVAILABLE_GPUS}")
    ...
    components = 25     
    min_cluster = int(n_doc * 0.2/100)  ## adjust for the best cluster results
    min_sample = min_cluster    
    min_topic_size = min_cluster  # Use the same value for min_topic_size as min_cluster_size
    # min_topic_size = 15 if n_doc < 1000 else 25 if n_doc < 10**4 else 50 if n_doc < 10**5 else 100 if n_doc < 10**6 else 500 if n_doc < 10**7 else 1000
    print(f"Number of documents: {n_doc}, min_cluster_size: {min_cluster}")
    ...
    topic_model = get_topic_model(min_cluster, umap_model=umap_model, hdbscan_model=hdbscan_model)

    topics, probs = topic_model.fit_transform(docs, embeddings_array)

    ### step 3: outlier handling
    outliers_exist = -1 in topics
    if outliers_exist:
        print("Outliers detected, applying outlier reduction...")
        ...
        print("No outliers detected, skipping outlier reduction...")
        new_topics = topics
    ...
    bertopic_result = bertopic_result.drop(columns=['claim_text','topic'])
    
    # save topic info back to the original documents 
    bertopic_result_final = df.merge(bertopic_result, on='embed_id', how='left')
    print("Columns after merge:", bertopic_result_final.columns.tolist())
    if topics_to_drop: 
        bertopic_result_final = bertopic_result_final[~bertopic_result_final['topic'].isin(topics_to_drop)]
    
    # Save doctopic results as JSON
    doctopic_output_file = os.path.join(OUTPUT_PATH, f"5_claim2topic_doctopic_{DATASET}_{DATE}_v{VERSION}.json")
    ...
    topic_output_file = os.path.join(OUTPUT_PATH, f"5_claim2topic_topic_{DATASET}_{DATE}_v{VERSION}.json")
...
def visualize_cluster_metrics(evaluation_results, topic_model, output_path):
    ...
    # Save the figure
    cluster_metrics_file = os.path.join(output_path, f"5_cluster_metrics_{DATASET}_{DATE}_v{VERSION}.png")
...
def visualize_topic_distribution(topic_model, embeddings, output_path):
    ...
    # Exclude outliers (topic == -1) for color mapping and plotting
    ...
    reduced_embeddings = UMAP(n_neighbors=5, n_components=2,
                              min_dist=0.0, metric='cosine', random_state=42).fit_transform(embeddings)

    # Plot each topic with different colors (ignore outliers)
    for topic in unique_topics:
    ...
    if len(unique_topics) <= 15:
        ...
        # Show only first 15 topics in legend for readability
        handles, labels = plt.gca().get_legend_handles_labels()
        selected_handles = handles[:15]
        selected_labels = labels[:15]
        plt.legend(selected_handles, selected_labels, 
                   bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=9,
                   frameon=True, fancybox=True, shadow=True)

        # Calculate outlier percentage
        total_docs = len(topic_model.topics_)
        outlier_count = sum(1 for t in topic_model.topics_ if t == -1)
        outlier_pct = (outlier_count / total_docs) * 100 if total_docs > 0 else 0

        # Add note about legend and outlier percentage
        excluded_count = len(unique_topics) - len(selected_labels)
        plt.text(1.05, 0.02, 
                 f"Showing first {len(selected_labels)} topics\n"
                 f"{excluded_count} topics not shown\n"
    ...
    # Save the figure
    topic_dist_file = os.path.join(output_path, f"5_topic_distribution_{DATASET}_{DATE}_v{VERSION}.png")
